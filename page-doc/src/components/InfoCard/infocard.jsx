...
const InfoCard = ({ title, text, bgColor, downloadLink }) => (
     <div className={`p-8 shadow-md ${bgColor} text-white rounded-none w-full h-60 flex flex-col justify-center items-center`}>
       <h3 className="font-bold text-2xl text-center uppercase">{title}</h3>
       <p className="mt-4 text-lg text-center max-w-[80%]">{text}</p>
       {downloadLink && (
